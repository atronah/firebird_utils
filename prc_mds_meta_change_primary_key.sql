...
begin
    ...
    constraint_name = trim(coalesce(replace(constraint_name, '''', ''), left('pk_' || table_name, 31)));
    select list(part) from aux_split_text(replace(:fields_list, '''', ''), ',', 1) into fields_list;
