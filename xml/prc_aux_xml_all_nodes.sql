...
create or alter procedure aux_xml_all_nodes(
    ...
    , parent_namespaces blob sub_type text = null
)
returns(
...
begin
    ...
    if (parent_path not starts with '/') then
    ...
    ns_alias = '';
    content_offset = 0;
    namespaces = coalesce(parent_namespaces, '');
    ...
    begin
        ...
        begin
            ...
            if (attributes containing '=') then
            begin
                for select alias, name, val
                    from aux_xml_get_attributes(:attributes)
                    where alias = 'xmlns' or name = 'xmlns'
                    into namespace
                do
                begin
                    if (namespaces containing trim(coalesce(alias || ':', '')) || name || '="') then
                    begin
